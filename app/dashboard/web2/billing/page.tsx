...
import { getInvoicesByUser } from "@/lib/firestore";
...
export default async function BillingPage() {
  ...
  let invoices: Awaited<ReturnType<typeof getInvoicesByUser>> = [];
  ...
  try {
    if (session?.user?.id) {
      ...
      invoices = invRes.status === "fulfilled" ? invRes.value : [];
      projectCount = projRes.status === "fulfilled" ? projRes.value.length : 0;
